...
See an example of how you can utilize this workflow, together with jobs that do other checks below. For configuring the `JIRA_*` secrets see the [documentation of `create-jira-issues-for-community-activities`](CreateJiraIssuesForCommunityActivities.md), and for details on `MERGE_TOKEN` check out the workflow's inline documentation.
