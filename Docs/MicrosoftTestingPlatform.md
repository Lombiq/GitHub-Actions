...
3. Reference `Microsoft.Testing.Extensions.GitHubActionsReport` and `Microsoft.Testing.Extensions.TrxReport` 2.4.0 or later in each test project. Add `Microsoft.Testing.Extensions.HangDump` at the same version if using `blame-hang-timeout`. The updated Lombiq test SDK includes these packages. The SDKs in [Lombiq MSBuild Targets](https://github.com/Lombiq/MSBuild-Targets) set all of these up.
